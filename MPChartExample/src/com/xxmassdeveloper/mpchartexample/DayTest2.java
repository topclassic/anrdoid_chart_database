 ...
 */
public class DayTest2 extends DemoBase {
    ...
    protected void onCreate(Bundle savedInstanceState) {
    	Bundle extras = getIntent().getExtras();
  		...
  		elec_power = extras.getInt("elec_power");
	...
	private class ReadData extends AsyncTask<String, Void, Boolean>{
		...
		private ProgressDialog dialog = new ProgressDialog(DayTest2.this);
		...
		protected Boolean doInBackground(String... urls) {
			...
			try {
				...
				for(int i=0; i<jArray.length(); i++){
					...
					outlet.setPower(json.getInt("elec_power"));
		...
		protected void onPostExecute(Boolean result) {
			...
			if(result == false){
				Toast.makeText(DayTest2.this, error, Toast.LENGTH_LONG).show();
			}
			else{
    ...
    private LineData generateDataLine(int cnt) {
        ...
        for (int i = 0; i < 20; i++) {
            e1.add(new Entry( day + elec_power , i));
        }
        for (int i = 21; i < 31; i++) {
